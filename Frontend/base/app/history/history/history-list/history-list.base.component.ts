...
export class HistoryListBaseComponent{
...
scrollTop:number =0;
  ...
  "detailPageNavigation" : "click_of_the_row"
	...
	pageViewTitle: string = 'HISTORY_LIST';
  ...
  if (environment.prototype) {
  	gridSubscription = this.historyService.getProtoTypingData().subscribe((data: any) => {
	  	...
	  	let property: Exclude<keyof HistoryListBaseComponent, ''> = value;
	  	const method:any = "getChildTableData";
	  	let action: Exclude<keyof HistoryListBaseComponent, ''> = method;
	  	if(this.isChildPage && this[property] && typeof this[action] === "function"){
	  		params.pid = this[property];
	  		this[action](params);
	  	}
	  	else{
		  	this.historyService.getDatatableData(params).subscribe((data: any) => {
				let updateRecords: HistoryBase[] = [...this.gridData, ...data?.results];
...
this.subscriptions.push(scrollSubscription);
  ...
  this.tableSearchFieldConfig= this.appUtilBaseService.getControlsFromFormConfig(this.tableSearchConfig)
	...
	if (this.isChildPage && this[property]) {
		...
		let action: Exclude<keyof HistoryListBaseComponent, ''> = methodName;
		if (typeof this[action] == "function") {
			this[action]();
		}
	}
	else {
		this.router.navigate(['../historydetail'], { relativeTo: this.activatedRoute});
	}
}
...
this.loadGridData();
  ...
  this.showAdvancedSearch = !this.showAdvancedSearch;
	...
	this.historyService.getDatatableDataByPId(params).subscribe((data: any) => {
		let updateRecords: HistoryBase[] = [...this.gridData, ...data?.results];
		const ids = updateRecords.map(o => o.sid)
		this.gridData = updateRecords.filter(({ sid }, index) => !ids.includes(sid, index + 1))
		this.total = data?.filtered ? data?.filtered : 0;
...
onUpdateChild(id: any) {
 ...
 onNewChild() {
 	this.router.navigateByUrl('/history/historydetail' + '?pid=' + this.parentId)
 }
	initFilterForm(){
  ...
  if (this.selectedValues.length > 0) {
	...
	const requestedParams = {ids:values.toString()}
      this.confirmationService.confirm({
        ...
        accept: () => {
          this.historyService.delete(requestedParams).subscribe((res: any) => {
