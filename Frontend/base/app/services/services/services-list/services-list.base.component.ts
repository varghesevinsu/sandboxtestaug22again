...
export class ServicesListBaseComponent{
...
isRowSelected: boolean = false;
  ...
  "children" : [ {
    ...
    "children" : [ {
      ...
      "pageName" : {
        ...
        "sid" : "c7147491-4a7c-473c-8340-a9bf0004c978",
        "url" : "/services/servicesdetail"
      },
      "outline" : "true",
  ...
  "detailPageNavigation" : "click_of_the_row"
	...
	pageViewTitle: string = 'SERVICES_LIST';
...
this.subHeader = this.tableConfig.groupOnColumn?.name?.split('.');
  ...
  if (environment.prototype) {
  	gridSubscription = this.servicesService.getProtoTypingData().subscribe((data: any) => {
	  	...
	  	let property: Exclude<keyof ServicesListBaseComponent, ''> = value;
	  	const method:any = "getChildTableData";
	  	let action: Exclude<keyof ServicesListBaseComponent, ''> = method;
	  	if(this.isChildPage && this[property] && typeof this[action] === "function"){
	  		params.pid = this[property];
	  		this[action](params);
	  	}
	  	else{
		  	this.servicesService.getDatatableData(params).subscribe((data: any) => {
				let updateRecords: ServicesBase[] = [...this.gridData, ...data?.results];
  ...
  if (this.selectedValues.length > 0) {
	...
	const requestedParams = {ids:values.toString()}
      this.confirmationService.confirm({
        ...
        accept: () => {
          this.servicesService.delete(requestedParams).subscribe((res: any) => {
...
this.onRefresh();
	...
	this.servicesService.getDatatableDataByPId(params).subscribe((data: any) => {
		let updateRecords: ServicesBase[] = [...this.gridData, ...data?.results];
		const ids = updateRecords.map(o => o.sid)
		this.gridData = updateRecords.filter(({ sid }, index) => !ids.includes(sid, index + 1))
		this.total = data?.filtered ? data?.filtered : 0;
...
onUpdateChild(id: any) {
 ...
 onNewChild() {
 	this.router.navigateByUrl('/services/servicesdetail' + '?pid=' + this.parentId)
 }
	onRefresh(){
this.gridData = [];
this.params.start =0;
this.loadGridData();
  ...
  this.showAdvancedSearch = !this.showAdvancedSearch;
	...
	if (this.isChildPage && this[property]) {
		...
		this.router.navigate(['../servicesdetail'], { relativeTo: this.activatedRoute});
	}
}
