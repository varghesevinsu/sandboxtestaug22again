...
export class ToolDetailBaseComponent{
...
isRowSelected:boolean = true; 
  ...
  "type" : "form"
	...
	pageViewTitle: string = 'TOOL_DETAIL';
    ...
    let action: Exclude<keyof ToolDetailBaseComponent, ' '> = methodName;
...
addValidations(mandatoryFields:[]){
	...
	onSave(isToastNotNeeded?:boolean){
        ...
        if(!this.appUtilBaseService.isValidForm(this.detailFormControls, this.formErrors, finalArr, this.inValidFields)){
            ...
            this.messageService.clear();
            this.toolService[method](requestedObj).subscribe((res:ToolBase) => {
       ...
       if(environment.prototype && this.id){
        const params = {
          sid: this.id
            ...
            this.toolService.getProtoTypingDataById(params).subscribe((res:any) =>{
                this.data = res;
                this.backupData = res;
                this.detailFormControls.patchValue(this.backupData);
            });
            ...
            this.toolService.getById(params).subscribe((res:ToolBase[]) =>{
                this.data = res||{};
                this.backupData = res || {};
