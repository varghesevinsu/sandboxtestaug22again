...
export class UserGuideListBaseComponent{
...
isRowSelected: boolean = false;
  ...
  "detailPage" : {
    ...
    "sid" : "d6cadb54-6156-4cb3-b73f-95bbd75740b8",
    "url" : "/userguide/userguidedetail"
  },
  "detailPageNavigation" : "click_of_the_row"
}
	pageViewTitle: string = 'USER_GUIDE_LIST';
...
this.subHeader = this.tableConfig.groupOnColumn?.name?.split('.');
  ...
  if (environment.prototype) {
  	gridSubscription = this.userGuideService.getProtoTypingData().subscribe((data: any) => {
	  	...
	  	let property: Exclude<keyof UserGuideListBaseComponent, ''> = value;
	  	const method:any = "getChildTableData";
	  	let action: Exclude<keyof UserGuideListBaseComponent, ''> = method;
	  	if(this.isChildPage && this[property] && typeof this[action] === "function"){
	  		params.pid = this[property];
	  		this[action](params);
	  	}
	  	else{
		  	this.userGuideService.getDatatableData(params).subscribe((data: any) => {
				let updateRecords: UserGuideBase[] = [...this.gridData, ...data?.results];
...
this.subscriptions.push(scrollSubscription);
    ...
    let action: Exclude<keyof UserGuideListBaseComponent, ' '> = methodName;
  ...
  this.tableSearchFieldConfig= this.appUtilBaseService.getControlsFromFormConfig(this.tableSearchConfig)
	...
	if (this.isChildPage && this[property]) {
		...
		this.router.navigate(['../userguidedetail'], { relativeTo: this.activatedRoute});
	}
}
...
this.onRefresh();
	...
	this.userGuideService.getDatatableDataByPId(params).subscribe((data: any) => {
		let updateRecords: UserGuideBase[] = [...this.gridData, ...data?.results];
		const ids = updateRecords.map(o => o.sid)
		this.gridData = updateRecords.filter(({ sid }, index) => !ids.includes(sid, index + 1))
		this.total = data?.filtered ? data?.filtered : 0;
...
onUpdateChild(id: any) {
 ...
 onNewChild() {
 	this.router.navigateByUrl('/userguide/userguidedetail' + '?pid=' + this.parentId)
 }
	onRefresh(){
this.gridData = [];
this.params.start =0;
  ...
  if (this.selectedValues.length > 0) {
	...
	const requestedParams = {ids:values.toString()}
      this.confirmationService.confirm({
        ...
        accept: () => {
          this.userGuideService.delete(requestedParams).subscribe((res: any) => {
