...
export class ToolDesignTypeListBaseComponent{
...
isRowSelected: boolean = false;
  ...
  "children" : [ {
    ...
    "children" : [ {
      ...
      "type" : "button",
      "pageName" : {
        "name" : "Tool design type Detail",
        "sid" : "08384799-6041-45e8-b013-973f7b3f3937",
        "url" : "/tooldesigntype/tooldesigntypedetail"
      },
      "outline" : "true",
  ...
  "detailPageNavigation" : "click_of_the_row"
	...
	pageViewTitle: string = 'TOOL_DESIGN_TYPE_LIST';
	
		tableSearchControls : FormGroup = new FormGroup({
	enabled: new FormControl('',[]),
	toolDesignType: new FormControl('',[]),
});
	...
	this.toolDesignTypeService.getDatatableDataByPId(params).subscribe((data: any) => {
		let updateRecords: ToolDesignTypeBase[] = [...this.gridData, ...data?.results];
...
onUpdateChild(id: any) {
 ...
 onNewChild() {
 	this.router.navigateByUrl('/tooldesigntype/tooldesigntypedetail' + '?pid=' + this.parentId)
 }
	clearFilterValues() {
  ...
  this.onRefresh();
	...
	if (this.tableConfig.detailPage?.url) {
      const value: any = "parentId";
       let property: Exclude<keyof ToolDesignTypeListBaseComponent, ''> = value;
       const methodName: any = "onUpdateChild";
       let action: Exclude<keyof ToolDesignTypeListBaseComponent, ''> = methodName;
  ...
  if (this.selectedValues.length > 0) {
	...
	const requestedParams = {ids:values.toString()}
      this.confirmationService.confirm({
        ...
        accept: () => {
          this.toolDesignTypeService.delete(requestedParams).subscribe((res: any) => {
...
this.messageService.add(config);
	...
	if (this.isChildPage && this[property]) {
		...
		this.router.navigate(['../tooldesigntypedetail'], { relativeTo: this.activatedRoute});
	}
}
	onKeydown(event: any) {
...
this.loadGridData();
 ...
 this.onRefresh();
  ...
  if (environment.prototype) {
  	gridSubscription = this.toolDesignTypeService.getProtoTypingData().subscribe((data: any) => {
	  	...
	  	let action: Exclude<keyof ToolDesignTypeListBaseComponent, ''> = method;
	  	if(this.isChildPage && this[property] && typeof this[action] === "function"){
	  		params.pid = this[property];
	  		this[action](params);
	  	}
	  	else{
		  	this.toolDesignTypeService.getDatatableData(params).subscribe((data: any) => {
				let updateRecords: ToolDesignTypeBase[] = [...this.gridData, ...data?.results];
				const ids = updateRecords.map(o => o.sid)
				this.gridData = updateRecords.filter(({ sid }, index) => !ids.includes(sid, index + 1))
				this.total = data?.filtered ? data?.filtered : 0;
