...
export class LabDetailBaseComponent{
...
workflowActions:any ={
  ...
  "type" : "form"
	...
	pageViewTitle: string = 'LAB_DETAIL';
 ...
 onChangelog() {
  ...
  createAutoSuggestFields(ele: any) {
    ...
    let action: Exclude<keyof LabDetailBaseComponent, ' '> = methodName;
	...
	onSave(isToastNotNeeded?:boolean){
        ...
        if(!this.appUtilBaseService.isValidForm(this.detailFormControls, this.formErrors, finalArr, this.inValidFields)){
            ...
            this.messageService.clear();
            this.labService[method](requestedObj).subscribe((res:LabBase) => {
	...
	getData(){
        ...
        const params = {
          sid: this.id
            ...
            this.labService.getProtoTypingDataById(params).subscribe((res:any) =>{
                this.data = res;
                this.backupData = res;
                this.detailFormControls.patchValue(this.backupData);
            });
            ...
            this.labService.getById(params).subscribe((res:LabBase[]) =>{
                this.data = res||{};
                this.backupData = res || {};
