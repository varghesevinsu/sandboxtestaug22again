...
export class LabListBaseComponent{
...
isRowSelected: boolean = false;
  ...
  "detailPage" : {
    "name" : "Lab Detail",
    "sid" : "cb1ac777-116d-47bf-85f7-48d13e34d66b",
    "url" : "/lab/labdetail"
  },
  "detailPageNavigation" : "click_of_the_row"
}
	pageViewTitle: string = 'LAB_LIST';
...
this.subscriptions.push(scrollSubscription);
  ...
  if (this.selectedValues.length > 0) {
	...
	const requestedParams = {ids:values.toString()}
      this.confirmationService.confirm({
        ...
        accept: () => {
          this.labService.delete(requestedParams).subscribe((res: any) => {
  ...
  this.tableSearchFieldConfig= this.appUtilBaseService.getControlsFromFormConfig(this.tableSearchConfig)
	...
	let property: Exclude<keyof LabListBaseComponent, ''> = value;
	if (this.isChildPage && this[property]) {
		const methodName: any = "onNewChild";
		let action: Exclude<keyof LabListBaseComponent, ''> = methodName;
		if (typeof this[action] == "function") {
			this[action]();
		}
	}
	else {
		this.router.navigate(['../labdetail'], { relativeTo: this.activatedRoute});
	}
}
...
this.messageService.add(config);
  ...
  if (environment.prototype) {
  	gridSubscription = this.labService.getProtoTypingData().subscribe((data: any) => {
	  	...
	  	let action: Exclude<keyof LabListBaseComponent, ''> = method;
	  	if(this.isChildPage && this[property] && typeof this[action] === "function"){
	  		params.pid = this[property];
	  		this[action](params);
	  	}
	  	else{
		  	this.labService.getDatatableData(params).subscribe((data: any) => {
				let updateRecords: LabBase[] = [...this.gridData, ...data?.results];
...
this.loadGridData();
 ...
 this.onRefresh();
  this.toggleAdvancedSearch();
	...
	this.labService.getDatatableDataByPId(params).subscribe((data: any) => {
		let updateRecords: LabBase[] = [...this.gridData, ...data?.results];
		const ids = updateRecords.map(o => o.sid)
		this.gridData = updateRecords.filter(({ sid }, index) => !ids.includes(sid, index + 1))
		this.total = data?.filtered ? data?.filtered : 0;
...
onUpdateChild(id: any) {
 ...
 onNewChild() {
 	this.router.navigateByUrl('/lab/labdetail' + '?pid=' + this.parentId)
 }
