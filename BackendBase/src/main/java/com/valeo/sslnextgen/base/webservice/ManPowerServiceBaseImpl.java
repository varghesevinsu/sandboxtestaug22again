...
public abstract class ManPowerServiceBaseImpl<BL extends IManPowerBLBase<M>, M extends ManPowerBase>
		extends BaseWebService<BL, M> {
	
	
	public ManPowerServiceBaseImpl(BL logic) {
		super(logic);
	}
	@GET
	@Produces(MediaType.APPLICATION_JSON)
	...
	@Produces(MediaType.APPLICATION_JSON)
	@Path("/bypid/{pid}")
	public List<M> getManPowerByPId(@PathParam("pid") String pid) {
		return logic.getManPowerByPId(pid);
	}
