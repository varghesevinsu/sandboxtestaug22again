...
public abstract class UserGuideServiceBaseImpl<BL extends IUserGuideBLBase<M>, M extends UserGuideBase>
		extends BaseWebService<BL, M> {
	
	
	public UserGuideServiceBaseImpl(BL logic) {
		super(logic);
	}
	@PUT
	...
	@Produces(MediaType.APPLICATION_JSON)
	@Path("/bypid/{pid}")
	public List<M> getUserGuideByPId(@PathParam("pid") String pid) {
		return logic.getUserGuideByPId(pid);
	}
	...
	@Produces(MediaType.APPLICATION_JSON)
	@Path("bylink/{link}")
	public M getByLink(@PathParam("link") String link) {
		return logic.getByLink(link);
	}
