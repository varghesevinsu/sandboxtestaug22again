...
public abstract class ToolServiceBaseImpl<BL extends IToolBLBase<M>, M extends ToolBase>
		extends BaseWebService<BL, M> {
	
	
	public ToolServiceBaseImpl(BL logic) {
		super(logic);
	}
	@PUT
	...
	@Produces(MediaType.APPLICATION_JSON)
	@Path("/bypid/{pid}")
	public List<M> getToolByPId(@PathParam("pid") String pid) {
		return logic.getToolByPId(pid);
	}
	...
	@Produces(MediaType.APPLICATION_JSON)
	@Path("bytool/{tool}")
	public M getByTool(@PathParam("tool") String tool) {
		return logic.getByTool(tool);
	}
