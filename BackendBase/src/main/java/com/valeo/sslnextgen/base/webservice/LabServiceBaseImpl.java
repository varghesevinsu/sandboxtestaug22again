...
public abstract class LabServiceBaseImpl<BL extends ILabBLBase<M>, M extends LabBase>
		extends BaseWebService<BL, M> {
	
	
	public LabServiceBaseImpl(BL logic) {
		super(logic);
	}
	@POST
	@Path("/datatable/{pid}")
	...
	@Produces(MediaType.APPLICATION_JSON)
	@Path("/bypid/{pid}")
	public List<M> getLabByPId(@PathParam("pid") String pid) {
		return logic.getLabByPId(pid);
	}


	@GET
	@Produces(MediaType.APPLICATION_JSON)
	@Path("bylabname/{labname}")
	public M getByLabName(@PathParam("labname") String labName) {
		return logic.getByLabName(labName);
	}
