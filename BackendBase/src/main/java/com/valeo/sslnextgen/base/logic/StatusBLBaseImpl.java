...
public class StatusBLBaseImpl<T extends StatusBase> extends BaseBusinessLogic<T>
		implements IStatusBLBase<T> {
		
	private ChangelogBLBaseImpl changelogBL;
	
	public StatusBLBaseImpl(Class<T> modelClass) {
		super(modelClass);
	...
	public void onAfterSaveDB(T modelObj) {
	  changelogBL.createChangeLog("Status", modelObj.getSid(), Constants.SAVED, modelObj);
	}
	...
	public void onAfterUpdateDB(T modelObj) {
	  changelogBL.createChangeLog("Status", modelObj.getSid(), Constants.UPDATED, modelObj);
	}
	...
	public void onAfterDeleteDB(T modelObj) {
	  changelogBL.createChangeLog("Status", modelObj.getSid(), Constants.DELETED, modelObj);
	}	
	...
	public T getByStatusOfTheRequest(String statusOfTheRequest) {
		...
		filters.add(new SimpleFilter("statusOfTheRequest", statusOfTheRequest, Operator.EQUAL));
